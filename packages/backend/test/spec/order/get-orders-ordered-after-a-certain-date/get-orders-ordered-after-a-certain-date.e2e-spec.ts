...
describe('Get Orders Ordered after a Certain Date', () => {
    ...
    it('should return all orders which have a createdAt date > at given date if exist in DB', async () => {
        const getOrderResponse = await request(app.getHttpServer()).get('/api/orders/created-after/2000-10-19');

        expect(getOrderResponse.status).toBe(200);
    });

    it('should not return orders if date have not good format', async () => {
        const getOrderResponse = await request(app.getHttpServer()).get('/api/orders/created-after/testfakedate');
